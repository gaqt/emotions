...
void BlastEffect::clearEffects() { g_effects.clear(); }

bool BlastEffect::tick() { return m_age.tick(); }
...
void BlastEffect::draw() const {
   DrawRing(m_pos, m_age.m_ticks * BLAST_GROWTH,
            m_age.m_ticks * BLAST_GROWTH + 5, 0, 360, 20, PINK);
}

void BlastEffect::drawAll() {
   for (BlastEffect &e : g_effects)
      e.draw();
}
